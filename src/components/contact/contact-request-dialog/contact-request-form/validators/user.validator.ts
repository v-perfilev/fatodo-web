...
export const userValidator = (currentLogin: string, currentEmail: string): AsyncValidator =>
  new AsyncValidator(
    Yup.string()
      ...
      .matches(new RegExp('^(?!' + currentLogin + '$).*$'), {message: () => i18n.t('contact:addContact.fields.user.current')})
      .matches(new RegExp('^(?!' + currentEmail + '$).*$'), {message: () => i18n.t('contact:addContact.fields.user.current')}),
    {
      name: 'exists',
